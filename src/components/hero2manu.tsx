...
// Define the type for your main course menu items
interface MainCourseMenuItem {
  _id: string;
  name: string;
  price: number;
  calories: number;
...
const MainCourseMenu: React.FC = () => {
  const [menuItems, setMenuItems] = useState<MainCourseMenuItem[]>([]);
  const [loading, setLoading] = useState<boolean>(true);

  // Your GROQ query to fetch the main course menu data
  const query = `*[_type == "mainCorse"]{
    _id,
    price,
    calories,
    name,
  ...
  const fetchMenuItems = async () => {
    try {
      const data: MainCourseMenuItem[] = await client.fetch(query);
      setMenuItems(data);
      setLoading(false);
    } catch (error) {
      console.error("Error fetching main course menu items:", error);
    }
  };
  ...
  if (loading) {
    return <p className="text-center mt-10">Loading main course menu...</p>;
  }
